...
func setupTestDataForTest(t *testing.T) string {
	...
	filesToCreate := map[string]string{
		...
		"subdir/file3.txt": "Another file in subdirectory.",
	}
...
func TestGetPathSize_File(t *testing.T) {
	...
	expectedOutput := fmt.Sprintf("%s\t%s", humanizeSize(expectedSize), filePath)
...
func TestGetPathSize_DirectorySingleLevel(t *testing.T) {
	testdataPath := setupTestDataForTest(t)
	...
	expectedOutput := fmt.Sprintf("%s\t%s", humanizeSize(expectedTotalSize), dirPath)
...
func TestGetPathSize_DirectoryWithSubdirNotRecursive(t *testing.T) {
	testdataPath := setupTestDataForTest(t)
...
func TestGetPathSize_EmptyDirectory(t *testing.T) {
	...
	expectedOutput := fmt.Sprintf("%s\t%s", humanizeSize(expectedTotalSize), emptyDirPath)
...
func TestGetPathSize_NonExistentPath(t *testing.T) {
	...
	require.Error(t, err)
	require.Contains(t, err.Error(), "cannot acces path")
}
